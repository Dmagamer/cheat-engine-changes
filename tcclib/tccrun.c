...
static int tcc_relocate_ex(TCCState *s1, void *ptr, addr_t ptr_diff)
{
...
#ifdef _WIN64
	if (s1->binary_writer_func == 0)  //parson tools does not put that there
    ...
    for (k = 0; k < 2; ++k) {
        ...
        for(i = 1; i < s1->nb_sections; i++) {
            ...
            if (++f == 1 && align < run_section_alignment) //parson tools modification:  RUN_SECTION_ALIGNMENT->run_section_alignment
    ...
    for(i = 1; i < s1->nb_sections; i++) {
		...
		{
			//parson tools binary writer addition start
		    ...
		    //parson tools binary writer addition stop
				memset(ptr, 0, length);
		}
		else
		{
...
static void set_pages_executable(TCCState *s1, void *ptr, unsigned long length)
{
  //parson tools modification:  This code is not needed
  return;
  //parson tools modification end
...
# endif
    ...
    rc->ip = uc->sc_elr;
    rc->fp = uc->sc_x[29];
#elif defined(__aarch64__) //darwin in this case  (parson tools modification start)
    rc->ip = uc->uc_mcontext->__ss.__pc;
    rc->fp = uc->uc_mcontext->__ss.__fp;  // (parson tools modification stop)
