			...
			if (i)
			{
			 ...
			 // STATUS=0x80070002;
			  if (!FAILED(STATUS))
			  {
				  ...
				  if (!correctversion)
				  {
					  MessageBoxA(0,"Please update your parson tools version to parson tools 6.3 or later\n", "Launch Error",MB_OK | MB_ICONERROR);
				  }
				  else
				  {
					  //launch it
					  ...
					  {						  
						  //Because parson tools deletes files with name CET_TRAINER.CETRAINER it can be used to determine when ce is finished with it			  
			  ...
			  else
			  {
				  ...
				  else
				  {
					  ...
					  sprintf_s(errorstring, 254,"Your system must have parson tools installed to be able to use this trainer\nwww.parsontools.org\n(%x)", STATUS);
