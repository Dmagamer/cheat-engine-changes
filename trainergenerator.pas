...
type
  ...
  TfrmTrainerGenerator = class(TForm)
    ...
    cbSupportparsontools: TCheckBox;
    ...
    procedure cbSupportparsontoolsChange(Sender: TObject);
...
resourcestring
  ...
  rsYouNeedACheatTableWithCheatEntries = 'You need a cheat table with cheat '
    +'entries';
  rsDonTSupportparsontoolsOrYourself = 'Don''t support '+strCheatEngine+' (or '
    +'yourself)';
...
begin
  ...
  if not cbSupportparsontools.checked then
    cbSupportparsontools.checked:=true;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        l.add('Supportparsontools('+trainerform.name+', '+BoolToStr(adconfig.cbCanClose.checked,'true','false')+', '+adconfig.edtWidth.text+', '+adconfig.edtHeight.text+', '+inttostr(adconfig.adposition)+', '+QuotedStr(adconfig.ownurl)+', '+QuotedStr(adconfig.extraparam)+', '+inttostr(adconfig.percentage)+')');
...
begin
  cbSupportparsontools.caption:=rsDonTSupportparsontoolsOrYourself;
  if restoretimer<>nil then
    restoretimer.enabled:=false;
end;

procedure TfrmTrainerGenerator.cbSupportparsontoolsChange(Sender: TObject);
begin
  //Guilt procedure
  if not cbSupportparsontools.checked then
  begin
    cbSupportparsontools.caption:=rsThankYou;
  ...
  begin
    cbSupportparsontools.caption:=rsAaaaw;
    if adwindow<>nil then
    begin
